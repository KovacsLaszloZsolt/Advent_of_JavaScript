...
episodes.forEach((episod, index) => {
  ...
  // }
  li.innerHTML = `<a href='#'>
    <div class='episode'>Episode ${episod.id}</div>
    <div class='title'>${episod.title}</div>
  </a>`;
  ...
  li.addEventListener("click", () => {
    ...
    cover.innerHTML = `<img src="./images/${episod.cover}" alt="Episode ${episod.id}" />`;

    const content = document.querySelector(".content");
    content.innerHTML = `<h1>${episod.title}</h1>
        <p>${episod.description}</p>
        <a href="${episod.link}" target="_blank" class="more">More</a>`;
  });
  tabs.appendChild(li);
});
